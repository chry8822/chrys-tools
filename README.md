...
2단계  서버에서 fetch + pull 후 최신 커밋 확인
...
4단계  npm run build 실행

5단계  결과 출력

       ✅ 배포 완료
       서버: QA (192.168.1.100) / branch: develop / commit: a1b2c3d
...
| 서버 | 인증 | 사용 명령어 | 사전 조건 |
|------|------|-------------|-----------|
| QA | 비밀번호 | `plink -batch -pw` → 없으면 `sshpass` | PuTTY 또는 KiTTY 설치 필요 |
| CI | SSH 키 | `ssh` | Windows 10/11 내장 OpenSSH 사용, 별도 설치 불필요 |

> KiTTY / PuTTY를 이미 사용 중이라면 `plink`가 함께 설치돼 있으므로 추가 설정 없이 바로 동작합니다.

#### 경로 변경

배포 경로는 세 가지 방식으로 변경할 수 있습니다.

**1. 대화 중 즉석 변경** (해당 배포에만 적용)
```
"qa 배포하는데 경로는 /app/frontend야"
"이번엔 /app/front-v2로 배포해줘"
```
